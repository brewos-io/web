...

{
  ...
  "deviceKey": "base64url_encoded_32_byte_key"
}
```
...
- No user authentication required (the token itself is the secret)
- Token is hashed before storage (SHA-256)
- **Device key is also registered** - used for WebSocket connection authentication
- Expires in 10 minutes
- Rate limited (5 requests/minute per IP)

**Device Key:**

- Generated by ESP32 on first boot (cryptographically random, 32 bytes)
- Stored securely in ESP32 NVS (non-volatile storage)
- Hashed (SHA-256) before cloud storage
- Required for all subsequent WebSocket connections from the device
...
1. **Generation**: Cryptographically random key generated on ESP32 first boot
2. **Storage**: Persisted in secure NVS partition on ESP32
3. **Transmission**: Sent only once during pairing registration
4. **Validation**: Every WebSocket connection from device must include valid key
5. **Hash Storage**: Only hashed version stored in cloud database
...
| `/api/push/notify`            | 60 requests/minute         |

### Input Validation

All endpoints validate:

- Device ID format: `BRW-XXXXXXXX` (8 hex characters)
- Token formats and lengths
- Name/string field lengths (max 100 chars)
...
- Device WebSocket connections require valid device key
