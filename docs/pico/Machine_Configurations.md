...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   The SSR switches ON/OFF within a 1-second period.                         │
...
Parameters that can be changed without reflashing are stored in flash:

```c
typedef struct {
    uint32_t magic;             // 0xCAFE0001 (version marker)
    ...
    // Calibration offsets
    float    brew_temp_offset;
    float    steam_temp_offset;
...
Stored at fixed flash address, loaded at boot, updated via ESP32 commands.
